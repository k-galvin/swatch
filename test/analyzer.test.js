...
const semanticChecks = [
  ...
  ["complex layout", 'Layout "L1" size [100, 100] { Wall W from [0,0] to [10,10] }'],
  ["arithmetic", "let z = (5 + 5) * 2 / 3 % 4 ** 5 - 1;"],
  ["logical ops", "let b = true || false && !true;"],
  ["comparisons", "let c = 5 < 10 == true != (3 >= 2); let d = 1 > 0;"],
  ["repeat loop", "Layout L size [10, 10] { repeat 5 { place Chair at [0,0] } }"],
  ["range loop", "Layout L size [10, 10] { for i in 1...10 { place Table at [i, i] } }"],
  ["component and call", "component W(x: number) { Wall W1 from [0,0] to [x,x] } Layout L size [10,10] { W(5); }"],
  ...
  ["call in expression", "component F(x: number) { } Layout L size [1,1] { let y = F(5); }"],
];
...
const semanticErrors = [
  ...
  ["mismatched binary types", "let x = 1 + true;", /Operands do not have the same type/],
  ...
  ["redeclared param", "component C(x: number, x: number) {}", /Identifier x already declared/],
];
